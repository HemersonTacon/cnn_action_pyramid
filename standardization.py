...
'''
Retorna o numero de threads no arquivo @threads_file
'''
def get_threads():
...
'''
Retorna um np.array com o arquivo fc7 passado em @name
'''	
def read_fc7(name):
...
'''
Grava o np.array passado em @fc7 com o nome passado em @name
'''
...
'''
Retorna um np.array as cnnflows contidas no arquivo passado em @name
'''
...
'''
Grava o np.array passado em @cnnf com o nome passado em @name
'''
...
'''
Lista e le os arquivos dentro do diretorio passado em os.path.join(@indir, @set) com nomes terminados com a extensao .fc7
...
o outro com os nomes prontos para serem usados na gravacao dos arquivos (depois de processados) no diretorio de saida os.path.join(@outdir, @set)
'''
...
'''
Lista arquivos dentro do diretorio passado em os.path.join(@indir, @set) com nomes terminados com a extensao .cnnf
Eh esperado que dentro dessa pasta exsitam as pastas correspondentes aos niveis ate @level. Exemplo com @level igual a 4: nivel0, nivel1, nivel2, nivel3
...
'''
Grava todos arquivos fc7 do conjunto @set passados na lista @files com o os nomes passados na lista @names
'''
...
'''
Grava todos arquivos cnnf do conjunto @set passados na lista @files com um indice para cada @level com o os nomes passados na lista @names
'''
...
'''
Realiza normalizacao do tipo passado em @type dos arquivos em @indir com a extensao @ext
Os arquivos normalizados sao gravados no diretorio passado em @outdir
Caso a extensao seja do tipo '.cnnf' eh necessario passar tambem o @level
'''				
	...
	if type == 'max_min':
		...
		if ext == '.fc7':
			...
			for set in sets:
				...
				# Aplicando transformacao nos conjuntos de dados
			...
			for set in sets:
				...
				for k in range(level):
					...
					if set == 'training':
						try:
							# filtrando linhas que podem estar vazias e causar problemas
							...
							for item in in_files[k]:
								print("Shape de cada item do problema:",item.shape)
							sys.exit()
