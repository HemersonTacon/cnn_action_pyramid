...
def concat_base():
     ...
     for i in range (len(group)):
          ...
          print('vamo ver o que ta saindo aqui, loop das classes')
          print(full_file_path[i])
     ...
     for h in range(len(group)):
          ...
          for i in os.listdir(full_file_path[h]): # laço para o numero de arquivos que tem na pasta ( todas as pastas tem o mesmo numeor de arquivos)
               ...
               cnnFlow2 = pca.cnn_flow_para_PCA(cnnFlow)
               if not_first_time:
                    allCnnFlow = np.append(allCnnFlow,cnnFlow2,axis=0)
                    print('A dimensao de allCnnFlow é: ')
                    print(allCnnFlow.shape)
           
               else:
                    allCnnFlow = cnnFlow2     
         
               print('Estamos na iteracao numero: ')
               print(debug_cont)
               print('A dimensao de cnnFlow2 é: ')
               print(cnnFlow2.shape)
...
def _main(args):
     ...
     cnnFlow = concat_base()
     cnnFlow_conformed = pca.conform2PCA(cnnFlow)
